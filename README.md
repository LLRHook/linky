...
The hosted bot delivers eligible Erome originals as playable media galleries without re-encoding: MP4, up to 24 MiB, five minutes, 60 fps and native 1080p. Larger or incompatible media uses attachment preparation, with a 64 MiB input limit and compression when needed to fit Discord. Smaller sources stay at their original resolution. The first video is shown and the album stays; image-only albums are not yet supported. Availability and speed depend on the source, hosting services and Discord.

See [Erome previews](docs/erome-previews.md) for exact limits, caching, queueing and failure behavior, and [regional hosting](docs/erome-regional-hosting.md) for operator setup and measured results.
