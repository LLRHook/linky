...
The original stays because an album may include more videos or images. Input is limited to 64 MiB and five minutes; FFmpeg prepares an H.264/AAC MP4 at up to 720p and 9 MiB. Downloads have a two-minute deadline before conversion, so previews can take more than a minute to appear. One preparation runs at a time, with room for two more requests to wait their turn for up to five minutes. A full queue, an expired wait or an unavailable, protected, oversized or failed video leaves the album untouched. Image-only albums are not handled, and playback still depends on Discord. Attach Files permission is required.
