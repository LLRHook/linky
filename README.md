...
The original stays because an album may include more videos or images. Input is limited to 64 MiB and five minutes; FFmpeg prepares an H.264/AAC MP4 at up to 720p and 9 MiB. Downloads have a two-minute deadline before conversion, so a new album can take more than a minute to appear. One preparation runs at a time, with room for two more albums to wait their turn for up to five minutes. Requests for the same album share the work, and a finished preview can be reused from memory for five minutes. Manual previews show their processing stage. A full queue, an expired wait or an unavailable, protected, oversized or failed video leaves the album untouched. Image-only albums are not handled, and playback still depends on Discord. Attach Files permission is required.
...
Erome processing requests the selected album and its first video from Erome, without sending Discord message text or account/server IDs. Temporary input and output files are deleted after conversion, including on failure. Docker keeps `/tmp` in a bounded memory-backed mount, cleared when the container stops. Node-only installations should provide equivalent temporary storage and crash cleanup. To avoid repeating downloads and conversion, Linky keeps at most two successful MP4 previews and their canonical album URLs in process memory for five minutes, capped at 18 MiB of video data. This cache is never written to disk and clears on restart. The uploaded preview remains on Discord until removed. Linky does not keep a persistent media archive or log album/CDN URLs. See [Erome implementation and limits](docs/erome-previews.md).
