...
The original stays because an album may include more videos or images. Input is limited to 64 MiB and five minutes. Linky can preserve compatible H.264/AAC media with a known size, up to 1080p, when it fits the destination's upload allowance; otherwise it compresses to an MP4 up to 720p without increasing the source frame rate. It does not upscale lower-resolution videos to 1080p. The output budget normally leaves 1 MiB below Discord's allowance, up to a 63 MiB processing cap. Manual commands use the allowance Discord supplies for that interaction; automatic previews use the server's allowance. [Measured latency and delivery experiments](docs/erome-fast-embed-research.md) explain the remaining cold-download limit.
