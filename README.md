...
A fresh automatic replacement can notify people explicitly tagged in the original message when Discord recognized those tags. Linky never enables notifications from attribution, quoted replies, provider text, role tags or `@everyone`/`@here`. Reply mode, manual fixes, retries and refreshes add no mention notifications. The source's `@silent` setting is preserved. Discord's recipient settings control actual alerts, and the original may already have notified someone before replacement.
