...
When the operator enables regional hosting, Linky can return the unchanged original MP4 as a media gallery. This path accepts files up to 24 MiB, five minutes and 60 fps, with H.264/yuv420p video and AAC audio when present, at native resolution up to 1920×1080 or 1080×1920. It downloads six disjoint ranges through five Vercel regions and Hostinger, checks the complete file, and saves it before posting its URL. Smaller sources stay at their original resolution. The original album and the existing owner-only **Remove** behavior are preserved. [Regional hosting](docs/erome-regional-hosting.md) covers deployment status, storage and measured timings; a ten-second preview is not guaranteed.
