...
The original stays because an album may include more videos or images. Input is limited to 64 MiB and five minutes. Linky can preserve compatible H.264/AAC media with a known size, up to 1080p, when it fits the destination's upload allowance; otherwise it compresses to an MP4 while preserving source resolution up to 1920×1080, or 1080×1920 for portrait video. It does not upscale smaller sources or increase their frame rate. Compression can reduce quality to fit the output budget, which normally leaves 1 MiB below Discord's allowance, up to a 63 MiB processing cap. Manual commands use the allowance Discord supplies for that interaction; automatic previews use the server's allowance. [Measured latency and delivery experiments](docs/erome-fast-embed-research.md) explain the remaining cold-download limit.

For suitable MP4 files, encoding starts while the download arrives. Other files finish downloading first. A new album can still take more than a minute; recent previews can be reused for five minutes. One preparation runs at a time, with two waiting jobs and a five-minute queue deadline. Requests share work only when their album and output budget match. Manual previews show their processing stage. Failed or unavailable media keeps the album untouched. Image-only albums are not handled, playback still depends on Discord, and Attach Files permission is required.
...
Erome processing requests the selected album and its first video from Erome, without sending Discord message text or account/server IDs. Temporary input and output files are deleted after preparation, including on failure. Docker keeps `/tmp` in a bounded memory-backed mount, cleared when the container stops. Node-only installations should provide equivalent temporary storage and crash cleanup. Linky keeps at most two successful preview variants, their canonical album URLs and output budgets in process memory for five minutes, capped at 128 MiB of video data. The cache is never written to disk and clears on restart. Different upload budgets use separate variants. The uploaded preview remains on Discord until removed. Linky does not keep a persistent media archive or log album/CDN URLs. See [Erome implementation and limits](docs/erome-previews.md).
