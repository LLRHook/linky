...
export async function execute(interaction: ChatInputCommandInteraction, settings: Config, servers: ServerSettings): Promise<void> {
  ...
  await interaction.reply({
    content: [
      ...
      'Erome previews the first video as an MP4 reply and keeps the album. Server admins can allow ordinary channels with /settings erome_channels:all; the default is age-restricted channels. DMs are excluded. Limits: 64 MiB input and five minutes. Downloads and conversion can take more than a minute. Only the original sharer or manual requester can use Remove.',
