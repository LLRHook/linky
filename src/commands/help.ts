...
export async function execute(interaction: ChatInputCommandInteraction, settings: Config, servers: ServerSettings): Promise<void> {
  ...
  await interaction.reply({
    content: [
      ...
      'Automatic Erome previews follow Replace or Reply mode. Original post opens the full album; hosted galleries offer requester-only Load next item. Failed previews, manual fixes and retries keep the source. Server admins can allow ordinary channels with /settings erome_channels:all; the default is age-restricted channels. DMs are excluded. Limits: 64 MiB input and five minutes. Slow jobs show progress; Details privately explains your delivery. Only the original sharer or manual requester can use Remove.',
