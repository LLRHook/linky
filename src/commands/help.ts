...
export async function execute(interaction: ChatInputCommandInteraction, settings: Config, servers: ServerSettings): Promise<void> {
  ...
  await interaction.reply({
    content: [
      ...
      'Erome keeps the album and previews its first video or supported image. Hosted galleries offer requester-only Load next item. Server admins can allow ordinary channels with /settings erome_channels:all; the default is age-restricted channels. DMs are excluded. Limits: 64 MiB input and five minutes. Slow jobs show progress; Details privately explains your delivery. Only the original sharer or manual requester can use Remove.',
