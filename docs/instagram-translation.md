...
The English caption excerpt appears with a small source-language label and the provider's native gallery preview in one message. If Instagram7's gallery fails, Linky tries `g.oginstagram.com` on the same output. Gallery modes remain aliases of their providers, with one bounded fallback; a normal embed containing the original caption cannot count as a translated-preview success. [OGInstagram gallery documentation](https://github.com/seirenkr/OGInstagram/blob/87110e42eb4b4b99ef4ec09c97c6971e95f97ce7/README.md).

Captions are displayed as one paragraph of up to 300 characters, including an ellipsis when shortened. Markdown escaping counts toward the limit; emoji clusters and caption URLs are not split. The excerpt can be shorter when the surrounding message needs room. No full-caption text file is added; **Original post** opens the source caption. The complete available caption is still sent for translation. Native captions shown without translation remain controlled by the provider. A failed single-post preview keeps the original message and a tracked English-caption output. A mixed/multiple-post preview failure uses the existing rollback path. An alternate URL that would exceed Discord's message limit is skipped so the caption excerpt and failure notice can still fit. Only captions are translated: no image OCR, audio transcription or replacement media is produced. Manual `/fix` commands are unchanged, and messages already using rich translated X cards skip Instagram translation to preserve media.
...
6. Confirm long captions have a short excerpt and a visible language label without a text attachment, including captions with emoji or a URL longer than the excerpt. Confirm caption URLs never create extra embeds or override the gallery preview expectation.
