...
An unavailable preview keeps its original. Where ownership can be saved, automatic failures offer **Retry preview**; any channel member with current view/send permission who is not timed out can retry. A shared retry keeps the original attribution and Remove ownership, and the existing per-source cooldown still applies. **Remove** remains restricted to the original sharer for automatic previews and the requester for manual previews. Discord's native moderation is unchanged. YouTube community posts can share a message with other community posts, up to five sources and ten cards. A message that also requests a supported native fixer, YouTube video or Erome preview keeps its original and receives guidance to share those posts separately. No provider/media lookup or fallback runs for this unsupported combination, and the guidance has no Retry button. Unrelated, hidden or disabled-platform links do not count as requested preview coverage. Manual mixed requests receive private guidance.
...
Mixed community guidance uses the finite `unsupported` delivery outcome and remains separate from `metadata-unconfirmed` and provider recovery evidence. It is included in the archive's all-finalized-attempt denominator; it is not a successful preview or a provider failure.
