...
- A two-second HEAD request establishes the exact size and strong ETag. Six disjoint ranges then cover the file: five fixed Vercel regions and one Hostinger request. Each range is at most 4 MiB and has an eight-second deadline; the collector has a ten-second overall deadline including HEAD. Only one collector job is admitted at a time. These transfer limits exclude album lookup, queueing, inspection, storage and Discord delivery.
...
Regional tests additionally cover HMAC and one-use claims, six-part assembly, stalled reads and cancellation races, original-file inspection, persistent quotas and references, and HTTP file serving. The [hosting guide](erome-regional-hosting.md#measured-results-and-pending-validation) records the later regional experiments. Their source was already resolved; generic production validation is still pending.
