...
- Compatible H.264/yuv420p video and AAC audio up to 1080p can be remuxed without re-encoding when the source has a known size and fits. Otherwise the encoder preserves source resolution and aspect ratio up to 1920×1080, or 1080×1920 for portrait video, without upscaling smaller sources. It avoids raising the source frame rate and uses the output budget for video and audio. Compression can reduce quality; it never truncates a clip to fit.
